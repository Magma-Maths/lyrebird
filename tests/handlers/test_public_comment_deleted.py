"""Tests for public_comment_deleted handler."""

from __future__ import annotations

from unittest.mock import MagicMock

from lyrebird.handlers.public_comment_deleted import handle
from tests.conftest import make_comment_payload, make_mock_comment, make_mock_issue


def test_tombstones_mirrored_comment(config, mock_client):
    ...
    assert "deleted on public" in new_body


def test_no_mirrored_comment_to_tombstone(config, mock_client):
    payload = make_comment_payload(comment_id=999)
